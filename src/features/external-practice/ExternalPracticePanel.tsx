...
function summarizeExternalPractice(
  ...
  return [...grouped.entries()]
    .map(([taskNumber, values]) => {
      const ordered = [...values].sort(
        (left, right) =>
          right.data.practicedAt.toMillis() - left.data.practicedAt.toMillis(),
      );
      const latest = ordered[0]!;
      return {
        taskNumber,
        attempts: values.length,
        solvedQuestions: values.reduce(
          (total, value) => total + value.data.maxScore,
          0,
        ),
        averageAccuracy: Math.round(
          values.reduce((total, value) => total + value.data.accuracy, 0) /
            values.length,
        ),
...
export function ExternalPracticePanel({
  ...
  additionalAttempts?: Array<DocumentWithId<ExternalPracticeAttempt>>;
}) {
  const attempts = useExternalPracticeAttempts(teacherId, studentId);
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [deleteError, setDeleteError] = useState<string | null>(null);
  const combinedAttempts = useMemo(
    () => [...attempts.data, ...additionalAttempts],
    [additionalAttempts, attempts.data],
  );
  const summary = useMemo(
    () => summarizeExternalPractice(combinedAttempts),
    [combinedAttempts],
  );
  ...
  async function removeAttempt(id: string, attempt: ExternalPracticeAttempt) {
    if (
      !window.confirm(
        `Удалить попытку №${attempt.taskNumber} с результатом ${attempt.score}/${attempt.maxScore}?`,
      )
    )
      return;
    setDeletingId(id);
    setDeleteError(null);
    try {
      await deleteExternalPracticeAttempt(getFirebaseDb(), {
        attemptId: id,
        teacherId,
        studentId,
      });
      ...
      setDeleteError(
        error instanceof Error ? error.message : "Не удалось удалить попытку.",
      );
    } finally {
      setDeletingId(null);
    }
  ...
  return (
    <section
      className="external-practice-panel"
      data-testid="external-practice-panel"
    >
      <header className="panel-heading">
        <div>
          <p className="eyebrow">Результаты практики</p>
          <h2>Практика по заданиям</h2>
          <p>
            Здесь объединены результаты из Русского100 и практических пунктов
            ДЗ.
          </p>
        </div>
      </header>
      {importEnabled ? (
        studentProgramId && examBlueprintId && examKind ? (
          ...
          <p className="shell-notice">
            Для импорта ученику нужна активная экзаменационная программа.
          </p>
        )
      ) : null}
      ...
      {attempts.loading ? (
        <p className="content-state">Загружаем практику…</p>
      ) : null}
      ...
      {summary.length ? (
        <div
          className="external-practice-summary"
          aria-label="Сводка практики по заданиям"
        >
          {summary.map((item) => (
            <article key={item.taskNumber}>
              ...
              <span>
                Попыток: {item.attempts} · вопросов: {item.solvedQuestions}
              </span>
              <span>
                Последняя: {item.lastScore}/{item.lastMaxScore}
              </span>
            </article>
          ))}
        </div>
...
function ManualImportForm({
  ...
  function prepare() {
    ...
    const unsupported = result.attempts.filter(
      (item) => !supported.has(item.taskNumber),
    );
    ...
    setErrors([
      ...
      ...unsupported.map(
        (item) =>
          `Задание №${item.taskNumber} отсутствует в активной программе и не будет импортировано.`,
      ),
    ]);
    setNotice("");
  }
  async function save() {
    const chosen = drafts.filter(
      (item) =>
        selected.has(practiceDraftKey(item)) && supported.has(item.taskNumber),
    );
    if (!chosen.length) {
    ...
    try {
      const result = await importExternalPracticeAttempts(getFirebaseDb(), {
        ...
        rows: chosen.map((item) => ({
          ...
          practicedAt: zonedLocalDateTimeToDate(
            item.localDate,
            item.localTime,
            timezone,
          ),
      ...
      setNotice(
        `Добавлено: ${result.added}. Уже было импортировано: ${result.skipped}.`,
      );
      setDrafts([]);
      setSelected(new Set());
      setText("");
      ...
      setErrors([
        error instanceof Error
          ? error.message
          : "Не удалось импортировать попытки.",
      ]);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="external-practice-import">
      <div className="external-practice-import__instructions">
        ...
        <p>
          Можно вставлять прямо из истории: «Задание №11: / 3/5 от 07.06.26
          13:57».
        </p>
      </div>
      ...
      {drafts.length ? (
        <div className="external-practice-preview">
          ...
          {drafts.map((item) => {
            ...
            return (
              <label className={!isSupported ? "is-disabled" : ""} key={key}>
                ...
                <span>
                  {item.localDate.split("-").reverse().join(".")} ·{" "}
                  {item.localTime}
                </span>
                <strong>
          ...
          >
            {saving
              ? "Импортируем…"
              : `Импортировать выбранное · ${selected.size}`}
          </button>
        </div>
      ) : null}
      {notice ? (
