...
export function LessonHomeworkBadge({ lesson, lessons, students }: {
  ...
  const description = `ДЗ не выдано: ${names.join(", ")}`;
  return <small className="lesson-homework-badge" title={description} aria-label={description}>
    <b className="lesson-homework-badge__full" aria-hidden="true">ДЗ не выдано{lesson.data.pairedLessonId ? ` · ${missing.length}/2` : ""}</b>
