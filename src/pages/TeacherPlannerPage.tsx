...
function PlannerTimeline({ date, items, lessons, students, timezone, onCreate, onDrop, onEdit, onToggle, onToggleLesson, onTogglePreparation }: {
  ...
  const events = [
    ...
    ...lessons.map((lesson) => ({
      ...
      title: `${lessonParticipantLabel(lesson.data, students)} · ${lesson.data.topic ?? "Урок"}`,
...
function PlannerMonth({ dates, focusDate, items, lessons, students, timezone, onOpenDay }: PlannerCalendarViewProps & { students: Array<DocumentWithId<Student>> }) {
  return <><div className="planner-month-weekdays">{weekdayLabels.map((day) => <span key={day}>{day}</span>)}</div><div className="planner-days">{dates.map((date) => {
    ...
    const highlights = [
      ...dayLessons.map((lesson) => ({ id: `lesson-${lesson.id}`, time: `${lessonTime(lesson.data, timezone)}–${lessonEndTime(lesson.data, timezone)}`, title: `${lessonParticipantLabel(lesson.data, students)} · ${lesson.data.topic ?? "Урок"}` })),
